...
#
SUBDIRS := Playground PolarClustering ShowerTopology 3DMichel G4Voxel Spallation HandClusterInput #ADD_NEW_SUBDIR ... do not remove this comment from this line
...
all:
	@for i in $(SUBDIRS); do ( echo "" && echo "Compiling $$i..." && cd $(LARLITE_USERDEVDIR)/CaraTool/$$i && $(MAKE) ) || exit $$?; done
#####################################################################################
#
# CLEANs...
#
clean:
	@for i in $(SUBDIRS); do ( echo "" && echo "Cleaning $$i..." && cd $(LARLITE_USERDEVDIR)/CaraTool/$$i && $(MAKE) clean && rm -f $(LARLITE_LIBDIR)/$$i.* ) || exit $$?; done
...
doxygen:
	...
	@mkdir -p $(LARLITE_USERDEVDIR)/CaraTool/doc/dOxygenMyProject
	@doxygen $(LARLITE_USERDEVDIR)/CaraTool/doc/doxygenMyProject.script
